...
#
#      Usage: GMT_Ash3d_to_gif.sh varID RunDir
#       e.g. /opt/USGS/Ash3d/bin/scripts/GMT_Ash3d_to_gif.sh             \
#               3                                                        \
...
#
SLAB="[GMT_Ash3d_to_gif.sh]: "            # Script label prepended on all echo to stdout
#
...
echo "${SLAB} running GMT_Ash3d_to_gif.sh with parameter:"
echo "${SLAB}   varID=$1"
varID=$1
if [ $varID -eq   0 ]; then
  echo "${SLAB}   0 = depothick"
...
if [ $varID -eq 0 ] || [ $varID -eq 1 ] || [ $varID -eq 2 ] || [ $varID -eq 3 ] ; then
  ...
  do
    ...
    if [ $varID -eq 0 ]; then
      # transient deposit
      gmt grdmath var.grd zero.grd AND = var_out_t${time}.grd
    else
  ...
  #gmt grdconvert "$ASH3D_NCFILE?depothickFin" var_out_final.grd
...
elif [ $varID -eq 4 ] || [ $varID -eq 7 ] ; then
  gmt grdconvert "$ASH3D_NCFILE?$var" var_out_final.grd
fi
...
do
  ...
  if [ $varID -eq 0 ] || [ $varID -eq 5 ] ; then
    ...
    echo "${SLAB} Plotting contours (var = varID1) for step = $t"
  ...
  if [ $varID -eq 1 ] || [ $varID -eq 2 ] || [ $varID -eq 3 ] || [ $varID -eq 7 ] ; then
  ...
  elif [ $varID -eq 0 ] || [ $varID -eq 4 ] || [ $varID -eq 5 ] ; then
    echo "${SLAB}  running legend_placer_dp"
...
EOF
    ...
    echo "${SLAB}  writing CL.txt for GMT 5/6"
    cat << EOF > CL.txt
> 0.25 1.25 14p 3i j
@%1%Dep. Arriv. Time
EOF
...
if [ "$CLEANFILES" == "T" ]; then
  echo "${SLAB} End of GMT_Ash3d_to_gif.sh: removing files."
...
echo "${SLAB}  "
echo "${SLAB} ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"
echo "${SLAB} finished GMT_Ash3d_to_gif.sh $varID $var"
echo `date`
echo "${SLAB} ++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++"

echo "${SLAB} exiting GMT_Ash3d_to_gif.sh with status $rc"
exit $rc
