...
if [ -z $1 ] ; then
    ...
    echo "Usage: runAsh3d_dp.sh rundir zipname dash_flag run_ID java_thread_ID"
    exit 1
  else
    RUNDIR=$1
...
ASH33DCFL=0.5 ${ASH3DEXEC} ${INFILE_PRELIM} 2>outerr.log | tee ashlog_prelim.txt
rc=$((rc + ${PIPESTATUS[0]}))
if [[ "$rc" -gt 0 ]] ; then
...
if [ "$RUNTYPE" == "ADV" ] || [ "$RUNTYPE" == "ACL" ]  ; then
    ...
    if [ "$USECONTAINERASH" == "T" ]; then
        ...
        ${ASH3DBINDIR}/Ash3d_PostProc pp_ashfalltime_shp.ctr 2>outerr.log
	    rc=$((rc + $?))
        if [[ "$rc" -gt 0 ]] ; then
