...
#
#      This script runs an Ash3d simulation for an advanced case launched
#      from the web interface at vsc-ash.wr.usgs.gov. It expects that the web client
#      has created a run directory containing the full control file ash3d_input.inp.
...
#       e.g. RT='ACL' /var/www/html/ash3d-api/htdocs/ash3druns/runAsh3d.sh    \
...
#
SLAB="[runAsh3d.sh]: "            # Script label prepended on all echo to stdout
#
...
NARGS=$#
echo "${SLAB} ------------------------------------------------------------"
echo "${SLAB} running runAsh3d.sh with $NARGS parameters:"
...
if [ -z $1 ] ; then
  echo "${SLAB}   Error: you must specify an input directory containing the file ash3d_input[_ac,_dp].inp"
  echo "${SLAB}   Usage: runAsh3d.sh rundir zipname dash_flag run_ID java_thread_ID"
  exit $rc
else
  RUNDIR=$1
...
do
  ...
  if [ "${plotvars[i]}" == "1" ]; then
    ...
    # Now check for shapefiles if needed.
    if [ $i -eq 6 ] ; then
      # Create a shapefile of the final deposit (inches)
      ${ASH3DBINDIR}/Ash3d_PostProc 3d_tephra_fall.nc 6 5
    elif [ $i -eq 5 ] ; then
    # Create a shapefile of the final deposit (mm)
