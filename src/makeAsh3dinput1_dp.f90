...
1041  format(i4,i2,i2,i2)
      ...
      read(linebuffer,*,iostat=iostatus) e_Height, e_Duration, e_Volume
      ...
      SimTime = e_Duration + 24.0_8

      if ((e_Hour.lt.0.0_8).or.((e_iyear.ne.0).and.(e_Hour.gt.24.0_8))) then
        write(error_unit,*) 'ERROR: Start hour must be between zero and 24.  You entered ',e_Hour
        stop 1
      endif
...
1006      format('Wind_nc/NCEP')
      ...
      if (Comp_Width.gt.360.0_8)then
        ! We should never actually be here for deposit cases
        Comp_Width = 360.0_8
        lonLL = 0.0_8
      endif
