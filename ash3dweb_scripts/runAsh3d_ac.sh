...
${ASH3DWEBSCRIPTDIR}/runGFS_traj.sh
${ASH3DWEBSCRIPTDIR}/GFSVolc_to_gif_ac_traj.sh 0
if [ "$ADVANCED_RUN" = "advanced1" ]; then
...
echo "Calling ${ASH3DWEBSCRIPTDIR}/GFSVolc_to_gif_tvar.sh 3"
${ASH3DWEBSCRIPTDIR}/GFSVolc_to_gif_tvar.sh 3

# Recreating the trajectory plot (using previously calculated trajecties), but using
# the consistant basemap
${ASH3DWEBSCRIPTDIR}/GFSVolc_to_gif_ac_traj.sh 1
...
if [[ $DASHBOARD_RUN == T* ]]
  then
    ...
    ${ASH3DWEBSCRIPTDIR}/GFSVolc_to_gif_tvar.sh 2
fi
rc=$((rc+$?))
echo "rc=$rc"
...
if [[ $DASHBOARD_RUN == T* ]]
  then
    ...
    ${ASH3DWEBSCRIPTDIR}/GFSVolc_to_gif_ac_puff.sh
fi
